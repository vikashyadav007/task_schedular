...
class ShowDefaultTask extends StatefulWidget{
  FirebaseUser _user;
  ShowDefaultTask(this._user);
  @override
  State<StatefulWidget> createState() {
    return ShowDefaultTaskState(this._user);
  }
}

class ShowDefaultTaskState extends State<ShowDefaultTask>{

  FirebaseUser _user;
  ShowDefaultTaskState(this._user);
  ...
  List<Widget> _getTaskTile(List list){
                 ...
                 PopupMenuItem(
                   ...
                   child: GestureDetector(
                     onTap: ()async{
                       ...
                       .document('tasks').updateData({list[i]:FieldValue.delete()})
                       .whenComplete((){
                          Navigator.of(context).push(MaterialPageRoute(builder: (context)=>ShowDefaultTask(_user)));
                       });   
                     },
    ...
    var data =  Firestore.instance.collection(_user.uid.toString()).document('tasks');
    data.get().then((value){
       if(value.data == null){
         setState(() {
           _isloading = false;
